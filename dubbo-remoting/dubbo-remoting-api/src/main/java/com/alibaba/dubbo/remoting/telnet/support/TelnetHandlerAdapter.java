...
/**
 * 实现 TelnetHandler 接口，继承 ChannelHandlerAdapter 类，telnet 处理器适配器，负责接收来自 HeaderExchangeHandler 的 telnet 命令，分发给对应的 TelnetHandler 实现类，进行处理，返回命令结果
 */
    ...
    public String telnet(Channel channel, String message) throws RemotingException {
        ...
        if (command.length() > 0) {
            ...
            if (extensionLoader.hasExtension(command)) { // 检查系统是否有命令对应的扩展点
                try {
                    String result = extensionLoader.getExtension(command).telnet(channel, message); // 交给具体扩展点去执行
                    if (result == null) {
                        return null;
                    }
                    buf.append(result);
            ...
            } else { // 查找不到对应的 TelnetHandler 对象，返回报错。
